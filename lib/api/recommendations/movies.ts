...
export async function fetchRecommendedMovies(params: RecommendationParams) {
  ...
  let baseUrl = `${BASE_URL}/discover/movie?include_adult=false&sort_by=popularity.desc&vote_count.gte=100`;
  ...
  try {
    ...
    if (results.length < 3) {
      const minimalUrl = industry 
        ? `${BASE_URL}/discover/movie?include_adult=false&sort_by=popularity.desc&vote_count.gte=100&with_original_language=${INDUSTRY_MAPPING[industry].language}`
        : `${BASE_URL}/discover/movie?include_adult=false&sort_by=popularity.desc&vote_count.gte=100`;
